/**
 * SameBuildingDifferentUnit â€” Interaction: address matches + floor differs
 *
 * Real case: Calle Mayor 5, Planta Baja vs Calle Mayor 5, Primera Planta
 * These are different units within the same building â€” NOT duplicates.
 *
...
export class SameBuildingDifferentUnit implements RuleInteraction {
  ...
  readonly description = "Address matches but floor differs â†’ different unit, not duplicate";
    ...
    return {
      ...
      reason: applied
        ? `Same address "${input.normalizedAddress}" but different floor ` +
          `("${input.normalizedFloor}" vs "${candidate.normalized_floor}") â†’ ` +
          `likely different unit in same building â†’ ${this.adjustment}pts`

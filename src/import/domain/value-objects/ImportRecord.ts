/**
 * Value Object: Registro de importaciÃ³n unificado
...
export interface NormalizedRecordData {
  ...
  accessRestriction?: string | null; // "true"/"false" â€” acceso restringido (maps to has_restricted_access)
  isPmrAccessible?: string | null; // "true"/"false" â€” accesible PMR / movilidad reducida
  ...
  // Dispositivo (desfibrilador fÃ­sico)
  ...
  isMobileUnit?: string | null; // "true"/"false" â€” el DEA se mueve (ambulancia, vehÃ­culo, etc.)
...
export class ImportRecord {
  ...
  /**
   * Crea un ImportRecord desde un registro de cachÃ© S3 que ya contiene datos normalizados
  ...
  get name(): string | null {
    ...
    // GeneraciÃ³n automÃ¡tica basada en externalId o direcciÃ³n
    // Si hay cÃ³digo externo vÃ¡lido (no generado automÃ¡ticamente), usarlo
    if (this.externalId && !this.isAutoGeneratedExternalId()) {
      return this.externalId;
    }

    // Generar nombre desde direcciÃ³n
    return this.generateNameFromAddress();
  }

  /**
   * Verifica si el externalId fue generado automÃ¡ticamente
   */
  ...
  private generateNameFromAddress(): string | null {
    ...
    // Si no tenemos suficiente informaciÃ³n, devolver null
    if (parts.length === 0) {
      return null;
    }
  ...
  get latitude(): number | null {
    ...
    if (!val) return null;
    // Soportar formato espaÃ±ol (coma) y estÃ¡ndar (punto)
  ...
  // Detalles de ubicaciÃ³n (combinaciÃ³n de additionalInfo y specificLocation)
  ...
  /**
   * Verifica si tiene los campos mÃ­nimos requeridos
   */
  ...
  /**
   * Detecta quÃ© campos han cambiado respecto a otro registro
   */
  ...
  private static applyFieldMappings(
    ...
    // Campos que deben tratarse como nÃºmeros (pueden venir con formato espaÃ±ol: coma como separador decimal)
    ...
    for (const [apiField, systemField] of Object.entries(mappings)) {
      ...
      if (value !== undefined && value !== null) {
        ...
        // Convertir formato espaÃ±ol (coma) a formato estÃ¡ndar (punto) para campos numÃ©ricos
  ...
  /**
   * Calcula hash del contenido para detecciÃ³n de cambios
   */
